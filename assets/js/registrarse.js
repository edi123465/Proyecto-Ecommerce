document.getElementById('registerForm').addEventListener('submit', async function (e) {
    ...
    const checkDuplicateResponse = await fetch('http://localhost:8088/Milogar/Controllers/UsuarioController.php?action=checkDuplicate', {
        method: 'POST',
        headers: {
    ...
    try {
        ...
        const response = await fetch('http://localhost:8088/Milogar/Controllers/UsuarioController.php?action=suscribirse', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (response.ok) {
            ...
            window.location.href = 'http://localhost:8088/Milogar/index.php'; // Redirigir a la página principal
        } else {
