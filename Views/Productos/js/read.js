...
function cargarProductos(page = 1, search = '') {
    ...
    fetch(`http://localhost:8088/Milogar/Controllers/ProductoController.php?action=obtenerTodo&page=${page}&limit=${limit}&search=${encodeURIComponent(search)}`)
...
function searchProducts() {
    ...
    if (query.length > 0) {
        ...
        fetch(`http://localhost:8088/Milogar/Controllers/ProductoController.php?action=searchProducts&q=${encodeURIComponent(query)}`)
                ...
                if (data && data.length > 0) {
                    ...
                    data.forEach(product => {
                        const imagenUrl = product.imagen && product.imagen.startsWith('http')
                            ? product.imagen
                            : 'http://localhost:8088/Milogar/assets/imagenesMilogar/Productos/' + (product.imagen || 'default.png');
